...
public class DashboardCombined1Fragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if(dataList.size() != 0){
            ...
            xLabels.setLabelCount(process.length);
            ...
            for(int i = 0 ; i < process.length; i++){
                possibility_bar_entries.add(new BarEntry(i, (float) possibility_score.get((String) process[i])/ total_size.get((String) process[i])));
                severity_bar_entries.add(new BarEntry(i, (float)severity_score.get((String) process[i])/ total_size.get((String) process[i])));
            }
            ...
            severity_set.setAxisDependency(YAxis.AxisDependency.LEFT);
            severity_set.setDrawValues(false);

            float groupSpace = 0.2f;
            float barSpace = 0f;
            float barWidth = 0.4f;

            BarData barData = new BarData(possibility_set, severity_set);
         ...
         */
            ...
            for(int i = 0 ; i < process.length; i++){
                risk_score_entries.add(new Entry(i + 0.5f, (float) risk_score.get((String) process[i])/ total_size.get((String) process[i])));
            }
